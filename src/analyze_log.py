...
def joao_didnt_ask(path_to_file, name):
    data = csv_to_list(path_to_file)
...
def joao_didnt_go(path_to_file, name):
    data = csv_to_list(path_to_file)
    all_days = set()
    joao_days = set()
    for i in range(len(data)):
        all_days.add(data[i][2])  # guarda o todos os dias
        if data[i][0] == name:  # dias do joão
...
def analyze_log(path_to_file):
    with open("./data/mkt_campaign.txt", mode="w") as arquivo:
        ...
        arquivo.write(f"{joao_didnt_ask(path_to_file, 'joao')}\n")
        arquivo.write(f"{joao_didnt_go(path_to_file, 'joao')}\n")
