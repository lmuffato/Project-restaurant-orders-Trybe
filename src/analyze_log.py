...
def count_orders_by_person(path_to_file, person, order):
    with open(path_to_file) as csvfile:
        data = csv.reader(csvfile)
        personsOrders = list()
...
def never_ordered_by_person(path_to_file, person):
    with open(path_to_file) as csvfile:
        data = csv.reader(csvfile)
        personsOrders = set()
        menu = set()
...
def get_days_not_frequented_by_person(path_to_file, person):
    with open(path_to_file) as csvfile:
        data = csv.reader(csvfile)
        frequented_days = set()
        schedule = set()
...
def analyze_log(path_to_file):
    ...
    neverOrderedByJoao = never_ordered_by_person(path_to_file, 'joao')
