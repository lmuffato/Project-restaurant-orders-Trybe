...
def most_ordered_food_per_customer(csv_data, customer_name):
    ...
    return collections.Counter(foods).most_common(1)[0][0]
...
def most_ordered_frequency_per_customer(csv_data, food, customer_name):
    ...
    for order in csv_data:
        if order[0] == customer_name:
            if order[1] == food:
                foods.append(food)
    return collections.Counter(foods).most_common(1)[0][1]
...
def never_ordered_food_per_customer(csv_data, customer_name):
    ...
    return foods.difference(customer_ordered)
...
def analyze_log(path_to_file):
    ...
    food_per_customer = most_ordered_food_per_customer(
        csv_data, 'maria'
    )
    ...
    customer_never_went = never_went_per_customer(
        csv_data, 'joao'
    )
